...
class IRNN(tf.keras.Model):
    ...
    def __init__(self, rnn_units=128, n_queries=50, kl_power=-3, op_scale=0.05,
        ...
        def hierarchical_prior(shape, name, initializer, pi, scale = None, project = False, n_samples = n_samples, regularizer=None, constraint=None, cashing_device=None, dtype=tf.float32):
            if scale == None:
                ...
                cat=tfd.Categorical(
                    ...
                    components = [
                        tfd.MultivariateNormalDiag(
                            ...
                            scale_diag = 1e-5 + q_scale*tf.nn.softplus(c + t[0])),
                        tfd.MultivariateNormalDiag(
                            loc = tf.zeros(t[0].shape[1:]), 

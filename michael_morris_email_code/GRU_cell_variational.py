...
class GRU_Cell_Variational(DropoutRNNCellMixin, base_layer.BaseRandomLayer):
    ...
    def get_kernel(self, inputs):
        ...
        # If using multiple samples  
        if self.project == 1:
            batch_size = inputs.shape[0]
            self.kernel = tf.repeat(self.kernel, int(tf.math.ceil(batch_size / self.n_samples)), 0)[:batch_size]
            self.recurrent_kernel = tf.repeat(self.recurrent_kernel, int(tf.math.ceil(batch_size / self.n_samples)), 0)[:batch_size]

            if self.use_bias:
                self.bias = tf.repeat(self.bias, int(tf.math.ceil(batch_size / self.n_samples)), 0)[:batch_size]
        return
    ...
    def call(self, inputs, states, training=None):
        ...
        if not self.project:
            ...
            matrix_x = tf.einsum('ij,ijk->ik', inputs, self.kernel)
            if self.use_bias:
                matrix_x = tf.add(matrix_x, input_bias)
        else:
            inputs_shape = tf.shape(inputs)            
            matrix_x = tf.map_fn(fn = lambda x: backend.dot(x[0], x[1]), elems = (split_first(inputs, self.n_samples), self.kernel), dtype=tf.float32)
            if self.use_bias:
                matrix_x = tf.map_fn(fn = lambda x: backend.bias_add(x[0], x[1]), elems = (matrix_x, input_bias), dtype=tf.float32)
            matrix_x = tf.reshape(matrix_x, (-1, tf.shape(matrix_x)[-1]))[:inputs_shape[0]]
        ...
        if self.reset_after:
            ...
            if not self.project:
                ...
                matrix_inner = tf.einsum('ij,ijk->ik', h_tm1, self.recurrent_kernel)
                if self.use_bias:
                    matrix_inner = tf.add(matrix_inner, recurrent_bias)
            else:
                matrix_inner = tf.map_fn(fn = lambda x: backend.dot(x[0], x[1]), elems = (split_first(h_tm1, self.n_samples), self.recurrent_kernel), dtype=tf.float32)
                if self.use_bias:
                    matrix_inner = tf.map_fn(fn = lambda x: backend.bias_add(x[0], x[1]), elems = (matrix_inner, recurrent_bias), dtype=tf.float32)
                matrix_inner = tf.reshape(matrix_inner, (-1, tf.shape(matrix_inner)[-1]))[:inputs_shape[0]]
            ...
            if not self.project:
                ...
                matrix_inner = tf.einsum('ij,ijk->ik', h_tm1, self.recurrent_kernel[:, : 2 * self.units])
            else:
                h_tm1_split = split_uneven(h_tm1, self.n_samples)
                matrix_inner = [backend.dot(_h_tm1, kernel[:, : 2 * self.units]) for _h_tm1, kernel in zip(h_tm1_split, self.recurrent_kernel)]
                matrix_inner = tf.concat(matrix_inner,0)

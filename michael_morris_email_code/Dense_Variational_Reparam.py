...
class DenseVariational_repareterisation(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        **kwargs: Extra arguments forwarded to `tf.keras.layers.Layer`.
        ...
        super(DenseVariational_repareterisation, self).__init__(
    ...
    def build(self, input_shape):
        ...
        if not self.project:
            shape = (last_dim + 1 if self.use_bias else 0, self.units )
        else:
            shape = (last_dim + 1 if self.use_bias else 0, self.units )
    ...
    def get_kernel(self, inputs):
        q = self._posterior(tf.random.normal([2,]))
        r = self._prior(tf.random.normal([2,]))

        self.kl_penalty = self._kl_divergence_fn(q, r)
        ...
        if self.project == 1:
            ...
            w = tf.repeat(w, int(tf.math.ceil(batch_size / self.n_samples)), 0)[:batch_size]
        else:
            w = tf.convert_to_tensor(value=q)

        if self.use_bias:
            self.kernel, self.bias = tf.split(w, [prev_units, 1], axis=-2)
            self.bias = tf.squeeze(self.bias)
        else:
            self.kernel, self.bias = w, None

    def call(self, inputs, first=True):
        dtype = tf.as_dtype(self.dtype or tf.keras.backend.floatx())
        ...
        if not self.project:
            ...
            outputs = tf.einsum('ij,ijk->ik', inputs, self.kernel)
            if self.use_bias:
